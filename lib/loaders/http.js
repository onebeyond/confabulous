var debug = require('debug')('confusion:loaders:http')
...
module.exports = function(_options, postProcessors) {
    ...
    var options = merge({}, { mandatory: true, request: { json: true, timeout: 10000, gzip: true, forever: false } }, _options)
    var exists = false
    var headers = {}
    ...
    return function(config, cb) {
        ...
        function watch(cb) {
            ...
            var watcher = setInterval(function() {
                head({ headers: { 'If-Modified-Since': headers['last-modified'] } }, function(err, res) {
                    ...
                    if (res.statusCode === 404 && exists || res.statusCode === 200 && (!exists || isModified(res))) {
        ...
        function unexpectedResponse(expected) {
            return !contains(expected, res.statusCode)
        }

        function isModified(res) {
            return _.find(headers, function(value, key) {
